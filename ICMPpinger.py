...
def checksum(string): 
    ...
    # Add carry bits to the checksum
...
def receiveOnePing(mySocket, ID, timeout, destAddr): 
    ...
    while True:
        ...
        # TODO: Fetch the ICMP header from the IP packet (Solution can be implemented in 6 lines of Python code).

        # Extracting ICMP header from IP packet
        icmpHeader = recPacket[20:28]  # Assume header is 8 bytes starting at byte 20
        icmpType, code, checksum, packetID, sequence = struct.unpack("bbHHh", icmpHeader)

        print(f"ICMP header extracted from IP packet...\n ICMP Type: {icmpType}\n Code: {code}\n Checksum: {checksum}\n Packet ID: {packetID}\n Sequence: {sequence}\n")

        if icmpType == 0:  # Echo reply
            return timeReceived - struct.unpack("d", recPacket[28:28 + struct.calcsize("d")])[0] # RTT = timeReceived - timeSent
